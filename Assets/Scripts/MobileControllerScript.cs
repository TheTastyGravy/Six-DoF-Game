...
public class MobileControllerScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < joysticks.Count; i++)
        {
            ...
            {
                // Clamp where the joystick button can be 
                float joystickX = Mathf.Clamp(joysticks[i].currentTouchPos.x - joysticks[i].touchOffset.x, joysticks[i].centerPoint.x - joysticks[i].movementRadius, joysticks[i].centerPoint.x + joysticks[i].movementRadius);
                float joystickY = Mathf.Clamp(joysticks[i].currentTouchPos.y - joysticks[i].touchOffset.y, joysticks[i].centerPoint.y - joysticks[i].movementRadius, joysticks[i].centerPoint.y + joysticks[i].movementRadius);
